...
mod_plugin = lightbulb.mod_plugin("Mod")
...
@lightbulb.option(
    "delete_message_days",
    "For how many days do the messages have to be deleted (0-7).",
    int,
    default=0,
)
...
async def cmd_timeout(ctx: lightbulb.context.SlashContext) -> None:
    ...
    await ctx.respond(
        f"Timed out `{member.display_name}` until <t:{int(communication_disabled_until.timestamp())}:f>."
    )
...
def load(bot):
    bot.add_mod_mod_plugin(mod_plugin)


def unload(bot):
    bot.remove_mod_mod_plugin(mod_plugin)
